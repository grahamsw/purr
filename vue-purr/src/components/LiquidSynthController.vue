  ...
  <div class="shell">
    <header>
      <div class="title-block">
        <h1>Liquid Resonator</h1>
        <p>WebAudio Physical Model</p>
      </div>
      <div class="transport">
        <div :class="['status-dot', isStarted ? 'alive' : '']"></div>
        <span class="status-label">{{ isStarted ? 'online' : 'offline' }}</span>
        
        <button :class="['gate-btn', isStarted ? 'on' : '']" @click="toggleSynth">
    ...
    <div v-if="error" class="error-banner">
      ...
      <p>Try refreshing the page or checking the console for more details.</p>
    </div>
    ...
    <div class="footer-hint" v-if="!isStarted">
      Click START to initialize the physical model and begin vibrating.
    </div>
    <div class="footer-hint" v-else-if="!isReady && !error">
      Loading AudioWorklet...
    </div>
  </div>
</template>
